...
namespace JobService.BackgroundServices
{
            ...
            try
            {
                ...
                foreach (var message in messages)
                {
                    try
                    {
                        ...
                        {
                            case "JobPosted":
                                var jobEvent = JsonSerializer.Deserialize<JobPostedEventDto>(message.Content);
                                if (jobEvent != null)
                                {
                                    publishResult = await messageBusService.PublishMessage(jobEvent, message.EventType);
                                }
                                break;
                            default:
