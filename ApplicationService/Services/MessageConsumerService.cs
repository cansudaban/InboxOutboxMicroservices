    ...
    public class MessageConsumerService : BackgroundService
    {
        ...
        public MessageConsumerService(IServiceScopeFactory scopeFactory, ILogger<MessageConsumerService> logger)
        {
            ...
            try 
            {
                ...
                _deadLetterQueueName = "dead_letter_queue_application";
                ...
                _queueName = "application_service_queue";
                _channel.QueueDeclare(
                    queue: _queueName, 
                    durable: true, 
                    exclusive: false, 
        ...
        {
            _logger.LogInformation("Application MessageConsumerService started listening");
            ...
            consumer.Received += async (model, ea) =>
            {
                ...
                try
                {
                    ...
                    await _retryPolicy.ExecuteAsync(async () => 
                    {
                        ...
                        {
                            await ProcessJobPostedEvent(jobPostedEvent, dbContext);
        ...
        private async Task ProcessJobPostedEvent(JobPostedEventDto jobEvent, ApplicationDbContext dbContext)
        {
            // When a job is posted, we don't automatically create applications
            // But we could track job postings or perform other business logic
            // For this example, we'll just log the job and mark it as tracked
            
            _logger.LogInformation($"Job posted: {jobEvent.JobTitle} at {jobEvent.CompanyName}");
            _logger.LogInformation($"Job ID: {jobEvent.JobId}, Location: {jobEvent.Location}");
            _logger.LogInformation($"Salary Range: {jobEvent.SalaryMin:C} - {jobEvent.SalaryMax:C}");
            _logger.LogInformation($"Required Skills: {string.Join(", ", jobEvent.RequiredSkills)}");
            
            // Here you could:
            // 1. Send notifications to subscribed candidates
            // 2. Index the job for search
            // 3. Track analytics
